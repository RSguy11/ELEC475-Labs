...
from model import AlexNetSnout

def load_alexnet_model(model_path, device):
    """Load the trained AlexNet model"""
    model = AlexNetSnout(pretrained=False).to(device)
    
    if os.path.exists(model_path):
        model.load_state_dict(torch.load(model_path, map_location=device))
        print(f"AlexNet model loaded from {model_path}")
        model.eval()
        return model
    else:
...
def plot_error_distributions(euclidean_distances, x_errors, y_errors, predictions, targets, model_condition):
    ...
    axes[0, 0].set_title(f'AlexNet {model_condition} - Error Distribution')
    axes[0, 0].legend()
    ...
    filename = f'alexnet_test_error_analysis_{model_condition.lower()}.png'
...
def visualize_best_worst_predictions(model, test_dataset, device, euclidean_distances, model_condition, num_samples=6):
    ...
    filename = f'alexnet_best_worst_predictions_{model_condition.lower()}.png'
    plt.savefig(f'{results_dir}/{filename}', dpi=300, bbox_inches='tight')
    plt.close()  # Close figure instead of showing to prevent stalling during automation

def main():
    """Main visualization function for AlexNet"""
    
    # Parse command line arguments
    parser = argparse.ArgumentParser(description='Visualize AlexNet-SnoutNet model results')
    ...
    print(f"Visualizing AlexNet-SnoutNet results on device: {device}")
    ...
    if args.type == 'baseline':
        model_paths = ['best_alexnet_model_baseline.pth']
    elif args.type == 'augmented':
        model_paths = ['best_alexnet_model_augmented.pth']
    else:  # auto-detect
        model_paths = [
            'best_alexnet_model_baseline.pth',
            'best_alexnet_model_augmented.pth'
        ]
    ...
    if model is None:
        print(f"No trained AlexNet model found! Please train the model first.")
        return
    ...
    print(f"\n[OK] AlexNet visualization completed! Check Results_Images/ for detailed analysis plots.")
