...
def train_alexnet_model(use_augmentation=False, num_epochs=50, batch_size=16, learning_rate=0.0001):
    """
    Training script for AlexNet-SnoutNet model.
    ...
    print(f"Training AlexNet-SnoutNet on device: {device}")
    
    # Hyperparameters
    print(f"Hyperparameters:")
    print(f"  Model: AlexNet-SnoutNet")
    ...
    # Initialize model
    model = AlexNetSnout(pretrained=True).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        model_filename = f'best_alexnet_model{aug_suffix}.pth'
...
def plot_training_curves(train_losses, test_losses, use_augmentation=False):
    ...
    plt.title(f'AlexNet-SnoutNet Training Progress{aug_suffix}', fontsize=16)
    ...
    aug_suffix = "_augmented" if use_augmentation else "_baseline"
    filename = f'alexnet_training_curves{aug_suffix}.png'
...
def evaluate_model(model, test_loader, device):
    ...
    print(f"\nFinal AlexNet Model Evaluation:")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Train AlexNet-SnoutNet model for pet nose localization')
    ...
    print("="*80)
    print("ALEXNET-SNOUTNET TRAINING CONFIGURATION")
    print("="*80)
    ...
    # Train the model
    trained_model, train_losses, test_losses = train_alexnet_model(
    ...
    print("\n[OK] AlexNet training completed!")
    
    aug_suffix = "_augmented" if use_augmentation else "_baseline"
    model_filename = f'best_alexnet_model{aug_suffix}.pth'
    plot_filename = f'alexnet_training_curves{aug_suffix}.png'
