...
def main():
    ...
    argParser.add_argument('-l', metavar='weight import', type=str, help='parameter file (.pth)')
    ...
    model.load_state_dict(torch.load(
        args.l, map_location=torch.device('cpu')))
    
    model.eval()

    part4output(img,model)
    part5output(img,model)
    part6output(model)

def part4output(img,model):
    print("Part 4 output")
    img_normalized = img.type(torch.float32) / 255.0

    with torch.no_grad():
        output = model(img_normalized.flatten())
